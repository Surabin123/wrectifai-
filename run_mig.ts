...
async function run() {
  try {
    ...
    for (const garage of res.rows) {
        ...
        if (text.includes('india') || text.includes('hyderabad') || text.includes('bengaluru') || text.includes('mumbai') || text.includes('delhi') || text.includes('kerala')) {
            country = 'IN';
            currency = 'INR';
            locale = 'en-IN';
        } else if (text.includes('uae') || text.includes('dubai') || text.includes('abu dhabi') || text.includes('sharjah')) {
            country = 'AE';
            currency = 'AED';
            locale = 'ar-AE';
        } else if (text.includes('japan') || text.includes('tokyo') || text.includes('osaka') || text.includes('kyoto')) {
            country = 'JP';
            currency = 'JPY';
            locale = 'ja-JP';
