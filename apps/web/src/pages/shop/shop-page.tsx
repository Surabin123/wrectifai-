            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {filteredProducts.map((product) => (
                <Card key={product.id} className="p-4 flex flex-col group relative rounded-[16px] border-slate-200 hover:shadow-lg transition-shadow">
                  ...
                  <div className="relative h-32 flex items-center justify-center mb-4 cursor-pointer" onClick={() => router.push(`/shop/${product.id}`)}>
                  ...
                  <h4 className="font-bold text-sm text-slate-900 line-clamp-2 h-10 mb-2 cursor-pointer hover:text-blue-600 transition-colors" onClick={() => router.push(`/shop/${product.id}`)}>{product.name}</h4>
