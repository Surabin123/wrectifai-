...
interface Customer {
  ...
  totalSpend: number;
  status?: string; // Optional if we wanted to mock it, but we'll use active
}
...
export default function GarageCustomersPage() {
  ...
  const loadData = async () => {
    try {
      const data = await apiClient.get<Customer[]>('/garages/my-customers');
      setCustomers(data || []);
    } catch (err) {
  ...
  const handleViewDetails = async (id: string) => {
    ...
    try {
      const data = await apiClient.get<any>(`/garages/my-customers/${id}`);
              ...
              <table className="w-full text-left border-collapse table-fixed min-w-[900px]">
                <thead>
                  <tr className="bg-slate-50/50 text-xs font-semibold text-slate-500 border-b border-slate-100">
                    ...
                    <th className="p-4 font-semibold w-[12%]">Last Visit</th>
                  ...
                  {loading ? (
                    ...
                    filtered.map((c) => (
                      <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="p-4 text-sm text-slate-700">
                          {c.lastVisit ? new Date(c.lastVisit).toLocaleDateString([], { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
                        </td>
          ...
          <Modal isOpen={!!selectedUser} onClose={() => setSelectedUser(null)} title="Customer Profile" className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
            {selectedUser?.loading ? (
              ...
              <div className="flex-1 overflow-y-auto pr-2 pb-4 space-y-6 custom-scrollbar">
                ...
                {/* Vehicles */}
                <div>
                  <h3 className="text-sm font-bold text-slate-900 mb-3 border-b pb-1">Vehicles Serviced Here</h3>
                  ...
                  ) : <p className="text-sm text-slate-500">No vehicles recorded for this garage.</p>}
                </div>
                ...
                <div>
                      ...
                      <tbody>
                        {selectedUser.bookings.map((b: any) => (
                          <tr key={b.id} className="border-b border-slate-100">
                            ...
                            <td className="py-2 px-3 font-medium">{b.amount ? `₹${b.amount}` : '-'}</td>
                  ...
                  ) : <p className="text-sm text-slate-500">No bookings made with this garage.</p>}
                </div>

                {/* Orders */}
                <div>
                  <h3 className="text-sm font-bold text-slate-900 mb-3 border-b pb-1">Parts/Inventory Orders</h3>
                      ...
                      <tbody>
                        {selectedUser.orders.map((o: any) => (
                          <tr key={o.id} className="border-b border-slate-100">
                            <td className="py-2 px-3 text-slate-700">{new Date(o.createdAt).toLocaleDateString()}</td>
                            <td className="py-2 px-3 text-slate-700">{o.garageName || 'Unknown'}</td>
                            <td className="py-2 px-3 font-medium">{o.amount ? `₹${o.amount}` : '-'}</td>
                            <td className="py-2 px-3">
                              <span className="text-xs px-2 py-0.5 rounded-full border bg-slate-50 uppercase text-slate-600">{o.status}</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ) : <p className="text-sm text-slate-500">No orders placed with this garage.</p>}
                </div>
